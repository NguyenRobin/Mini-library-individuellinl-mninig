...
btnSearchElem?.addEventListener('click', async function () {
  const dataBook: Book[] = await getData();
...
function createBookCover(book: Book[]): void {
...
function displayClickedBook(book: Book): void {
  const html = `
  ...
  <section class="book-cover">
    ...
    book.color
  };"><h3>${book.title}</h3><p>${book.author}</p></article>
  </section>
