...
function generateClickedBook(book: Book): void {
  const html = `
  ...
  <section class="book-cover">
    <article data-id="${
      book.id
    }" class="book-article book-size color" style="background-color: ${
    book.color
...
function createEachBook(book: Book[]): void {
  for (let i = 0; i < book.length; i++) {
    const articleElement = document.createElement('article');
    const h3Element: HTMLElement | null = document.createElement('h3');
    const paragraphElement: HTMLElement | null = document.createElement('p');
...
function clickedBook(books: Book[]) {
  const eachBookElem: NodeList = document.querySelectorAll('.book-article');
...
function clickEnter(books: Book[]) {
